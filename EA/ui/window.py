...
class MainFrame (wx.Frame):
    def __init__(self, parent, id, title, cf):
        wx.Frame.__init__(self, parent, id, title, wx.DefaultPosition, wx.Size(900,600),
                name=u'EasyAccout', style=wx.DEFAULT_FRAME_STYLE)
        config.cf = cf
    ...
    def OnFileNew(self, event):
        dlg = wx.FileDialog(
            ...
            defaultFile="", wildcard=_("EasyAccout Database (*.db)|*.db"), style=wx.SAVE)
    ...
    def OnLanguage(self, event):
        ...
        if ischange:
            wx.MessageBox(_('Language changed! You must restart EasyAccout !'), _('Note:'), wx.OK|wx.ICON_INFORMATION)

    def OnAboutInfo(self, event):
        info = wx.AboutDialogInfo()
        info.Name = u"EasyAccout"
        info.Version = version.VERSION
        info.Copyright = "(C) 2015"
        info.Description = wordwrap(_("EasyAccout is a personal finance software write by Python language.") + '\n',
            350, wx.ClientDC(self))
        info.WebSite = ()
        info.Developers = ["EasyAccout"]
