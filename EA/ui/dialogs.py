...
class UpdateDialog (MySizedDialog):
    def __init__(self, parent, version):
        ...
        wx.StaticText(panel, -1, _('Found new version:') + ' EasyAccout-%s' % (version))
        hl.HyperLinkCtrl(panel, wx.ID_ANY, _("Open download page"),URL=" ")
...
class UserCheckDialog (MySizedDialog):
    def __init__(self, parent):
        MySizedDialog.__init__(self, None, -1, 'EasyAccout ' + _('User Password'), 
                                style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
        self.parent = parent
        panel = self.GetContentsPane()
        panel.SetSizerType("form")
