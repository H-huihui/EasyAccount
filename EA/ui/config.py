...
class Configure:
    def __init__(self, charset='utf-8'):
        ...
        if sys.platform.startswith('win32'):
            ...
            self.confdir = os.path.join(os.environ['HOME'], '.EasyAccout')
            if not os.path.isdir(self.confdir):
                os.mkdir(self.confdir)

        self.conffile = os.path.join(self.confdir, "EasyAccout.conf") 
        ...
        self.datadef = {'lastdb':'', 'lang':'', #'rsa_pub':'', 'rsa_private':'', 
                     ...
                     'sync_ver':'', 'sync_auto': '', 'sync_md5':'', 'server':'EasyAccout.pythonid.com'}
    ...
    def load(self):
        ...
        try:
            ...
            self.iscreate = True
            self.data['lastdb'] = os.path.join(os.path.dirname(self.conffile), "EasyAccout.db")
    ...
    def default_db_path(self):
        return os.path.join(os.path.dirname(self.conffile), "EasyAccout.db")

    def lastdb_is_default(self):
        if self.data['lastdb'] == os.path.join(self.rundir, 'data', 'EasyAccout.db'):
            return True
        return False
